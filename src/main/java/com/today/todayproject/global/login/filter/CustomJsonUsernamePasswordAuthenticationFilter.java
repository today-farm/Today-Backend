package com.today.todayproject.global.login.filter;
...
/**
 * 스프링 시큐리티의 UsernamePasswordAuthenticationFilter를 참고하여 만든 커스텀 필터
 * 거의 구조가 같고, Type이 Json인 Login만 처리하도록 설정한 부분만 다르다. (API용)
 * Username : 회원 아이디 -> email로 설정
 */
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {

    private static final String DEFAULT_LOGIN_REQUEST_URL = "/login";  // "/login"로 오는 요청을 처리
    ...
    private static final String USERNAME_KEY="email"; // 회원 로그인 시 요청 JSON Key 이메일
    private static final String PASSWORD_KEY="password"; // 회원 로그인 시 요청 JSON Key 비밀번호
    ...
    public CustomJsonUsernamePasswordAuthenticationFilter(ObjectMapper objectMapper) {
        ...
        super(DEFAULT_LOGIN_PATH_REQUEST_MATCHER);   // 위에서 설정한  "/login"의 요청에, POST로 온 요청을 처리하기 위해 설정한다.
